...
const USE_MOCK =
  (process.env.NEXT_PUBLIC_USE_MOCK_DATA ?? "1").toString().toLowerCase() === "1" ||
...
const MARGIN_WARN_PCT = Number(process.env.NEXT_PUBLIC_MARGIN_WARN_PCT ?? 90);
...
function isTickMsg(x: unknown): x is TickMsg {
  return (
    isObject(x) &&
    (x as { type?: unknown }).type === "tick" &&
    ((x as { symbol?: unknown }).symbol === "NIFTY" || (x as { symbol?: unknown }).symbol === "BANKNIFTY") &&
    typeof (x as { ltp?: unknown }).ltp === "number" &&
    typeof (x as { t?: unknown }).t === "number"
  );
}
function isSignalMsg(x: unknown): x is SignalMsg {
  if (!isObject(x) || (x as { type?: unknown }).type !== "signal") return false;
  const inst = (x as { instrument?: unknown }).instrument;
  const payload = (x as { payload?: unknown }).payload;
...
function isRiskMsg(x: unknown): x is RiskMsg {
  return isObject(x) && (x as { type?: unknown }).type === "risk" && (typeof (x as { dailyLoss?: unknown }).dailyLoss === "number" || typeof (x as { dailyLoss?: unknown }).dailyLoss === "undefined") && (typeof (x as { marginUsedPct?: unknown }).marginUsedPct === "number" || typeof (x as { marginUsedPct?: unknown }).marginUsedPct === "undefined");
}
...
function calcPnLPct(side: Side, avgPrice: number, ltp: number) {
  ...
  return side === "Buy" ? ((ltp - avgPrice) / avgPrice) * 100 : ((avgPrice - ltp) / avgPrice) * 100;
}
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const candidates: unknown[] = [
        ...
        (mod as { LightweightCharts?: { createChart?: unknown } }).LightweightCharts?.createChart,
        (mod as { default?: unknown }).default,
        (mod as { default?: { createChart?: unknown } }).default &&
          (mod as { default: { createChart?: unknown } }).default.createChart,
      ];
      const create = candidates.find((c): c is CreateChartFn => typeof c === "function");
      if (!create) {
    ...
    return () => {
      const elLocal = el; // capture the element for cleanup
      const ro = resizeObsRef.current;
      if (ro && elLocal) {
        try {
          ro.unobserve(elLocal);
        } catch {
          /* noop */
        }
        ro.disconnect();
...
const SideBadge = ({ side }: { side: Side }) => (
  <span
    className={`px-2 py-0.5 text-xs rounded font-semibold ${
      side === "Buy" ? "bg-green-600/20 text-green-500" : "bg-red-600/20 text-red-500"
    }`}
  >
    {side.toUpperCase()}
  </span>
...
const TradeRow = React.memo(function TradeRow({
  ...
  return (
    <div className="grid grid-cols-6 items-center gap-2 px-3 py-2 text-sm border-b">
      <div className="truncate font-medium" title={t.symbol}>
        {t.symbol}
      </div>
      <div>
      ...
      <div className="text-right tabular-nums" title={`Status: ${t.status}`}>
        {formatINR(t.ltp)}
      </div>
...
function RiskBadge({ dailyLoss, marginUsedPct }: { dailyLoss: number; marginUsedPct: number }) {
  ...
  return (
    <span className="px-2 py-1 rounded text-xs font-semibold" title={title}>
      <span className={cls + " px-2 py-1 rounded"}>🟡 Risk</span>
    </span>
  );
}
...
export default function TradingDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.assert(/\\"x, y\\"/.test(JSON.stringify(csv)) || csv.includes('"x, y"'), "[TEST] CSV escaping comma");
      console.assert(csv.includes('"""z"""'), "[TEST] CSV escaping quotes");
    } catch (err) {
  ...
  // Stream hookup
  useEffect(() => {
    const teardown = createMarketStream(
      WS_URL,
      {
        ...
        onReconnect: (attempt, delay) =>
          setSysLogs((l) => pushBounded(l, `${new Date().toLocaleTimeString()} - reconnect attempt ${attempt} in ${Math.round(delay)}ms`, MAX_SYS_LOGS)),
        onMessage: (msg) => {
          if (msg?.type === "tick") {
  ...
  // Simulated risk drift (mock only)
  useEffect(() => {
    if (!USE_MOCK) return;
  ...
  // Chart series (epoch seconds)
  const seriesNifty = useMemo(() => {
    const base = Math.floor(Date.now() / 1000) - 60 * 5;
    const points = [22380, 22410, 22450, 22420, 22400, throttledMarket.nifty || 22400].slice(-5);
    return points.map((v, i) => ({ time: base + i * 60, value: Number(v) }));
  }, [throttledMarket.nifty]);

  const seriesBank = useMemo(() => {
    const base = Math.floor(Date.now() / 1000) - 60 * 5;
    const points = [48210, 48225, 48200, 48240, 48215, throttledMarket.bankNifty || 48210].slice(-5);
    return points.map((v, i) => ({ time: base + i * 60, value: Number(v) }));
    ...
    <div className="p-4 space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <Switch checked={showAbsPnl} onCheckedChange={(checked) => setShowAbsPnl(checked)} />
          <span className="text-sm">System</span>
          <Switch checked={isRunning} onCheckedChange={(checked) => setIsRunning(checked)} />
          {execBadge}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-lg font-semibold">{throttledMarket.nifty.toFixed(2)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Bank Nifty</div>
            <div className="text-lg font-semibold">{throttledMarket.bankNifty.toFixed(2)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Balance</div>
            <div className="text-lg font-semibold">{formatINR(500000)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">P&L</div>
            <div className="text-lg font-semibold">{formatINR(785.5)}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="nifty">
          ...
          <Tabs defaultValue="signals-n" className="mt-4">
                  ...
                  <Card key={idx} className="border-l-4" style={{ borderColor: sig.side === "Buy" ? "#16a34a" : "#dc2626" }}>
                    <CardContent className="p-4 space-y-1">
                      ...
                      <div className="text-sm opacity-90">
                        {sig.side} • {sig.qty} @ {formatINR(sig.price)}
                      </div>
            ...
            <TabsContent value="trades-n">
              <div className="flex items-center justify-between mb-2">
                ...
                <Button size="sm" onClick={() => exportTradesCSV("NIFTY")}>
                  Export CSV
                </Button>
              </div>
            ...
            <TabsContent value="logs-n">
              <div className="flex items-center justify-between mb-2">
                ...
                <Button size="sm" variant="secondary" onClick={() => exportLogsCSV("NIFTY")}>
                  Export CSV
                </Button>
              </div>
        ...
        <TabsContent value="banknifty">
          ...
          <Tabs defaultValue="signals-b" className="mt-4">
            ...
            <TabsContent value="trades-b">
              <div className="flex items-center justify-between mb-2">
                <div className="text-sm text-muted-foreground">Trades</div>
                <Button size="sm" onClick={() => exportTradesCSV("BANKNIFTY")}>
                  Export CSV
                </Button>
              </div>
              <div className="rounded border">
            ...
            <TabsContent value="logs-b">
              <div className="flex items-center justify-between mb-2">
                <div className="text-sm text-muted-foreground">Logs</div>
                <Button size="sm" variant="secondary" onClick={() => exportLogsCSV("BANKNIFTY")}>
                  Export CSV
                </Button>
              </div>
              <div className="h-48 overflow-auto rounded border p-2 text-xs space-y-1 bg-black/5">
        ...
        <TabsContent value="system">
          <div className="flex items-center justify-end mb-2">
            <Button size="sm" variant="secondary" onClick={() => exportLogsCSV("SYSTEM")}>
              Export CSV
            </Button>
          </div>
          <Card>
